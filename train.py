...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--enable_temporal_features', action='store_true', default=True,
                        help='Enable engineered temporal features (multi-window, smoothing, drift, decay).')
    parser.add_argument('--no_temporal_features', dest='enable_temporal_features', action='store_false',
                        help='Disable engineered temporal features.')
...
def main() -> None:
    ...
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        enable_temporal_features=args.enable_temporal_features,
    )
    ...
    model_args = {
        ...
        "user_dense_dim": pcvr_dataset.user_dense_schema.total_dim + (
            get_temporal_feature_dim(len(pcvr_dataset.seq_domains))
            if args.enable_temporal_features and pcvr_dataset.seq_domains else 0
        ),
