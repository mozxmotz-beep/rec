...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.task_type == "esmm_mmoe":
            self.experts = nn.ModuleList([
                nn.Sequential(nn.Linear(d_model, d_model), nn.SiLU(), nn.Dropout(dropout_rate))
                for _ in range(num_experts)
            ])
            self.gate_ctr = nn.Linear(d_model, num_experts)
            self.gate_cvr = nn.Linear(d_model, num_experts)
            self.ctr_head = nn.Linear(d_model, 1)
            self.cvr_head = nn.Linear(d_model, 1)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 5. Task head
        if self.task_type == "esmm_mmoe":
            expert_out = torch.stack([e(output) for e in self.experts], dim=1)  # (B,E,D)
            w_ctr = torch.softmax(self.gate_ctr(output), dim=-1).unsqueeze(-1)
            w_cvr = torch.softmax(self.gate_cvr(output), dim=-1).unsqueeze(-1)
            ctr_rep = (expert_out * w_ctr).sum(dim=1)
            cvr_rep = (expert_out * w_cvr).sum(dim=1)
            ctr_logit = self.ctr_head(ctr_rep)
            cvr_logit = self.cvr_head(cvr_rep)
            ctcvr_prob = torch.sigmoid(ctr_logit) * torch.sigmoid(cvr_logit)
            ctcvr_logit = torch.logit(ctcvr_prob.clamp(1e-6, 1 - 1e-6))
            return torch.cat([ctr_logit, cvr_logit, ctcvr_logit], dim=-1)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.task_type == "esmm_mmoe":
            expert_out = torch.stack([e(output) for e in self.experts], dim=1)
            w_ctr = torch.softmax(self.gate_ctr(output), dim=-1).unsqueeze(-1)
            w_cvr = torch.softmax(self.gate_cvr(output), dim=-1).unsqueeze(-1)
            ctr_rep = (expert_out * w_ctr).sum(dim=1)
            cvr_rep = (expert_out * w_cvr).sum(dim=1)
            ...
            ctcvr_prob = torch.sigmoid(ctr_logit) * torch.sigmoid(cvr_logit)
            ctcvr_logit = torch.logit(ctcvr_prob.clamp(1e-6, 1 - 1e-6))
            logits = torch.cat([ctr_logit, cvr_logit, ctcvr_logit], dim=-1)
        else:
