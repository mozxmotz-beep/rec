...
class PCVRParquetDataset(IterableDataset):
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        if self.enable_temporal_features and self.seq_domains:
            # Per domain: 6 window counts + 6 smoothed rates + drift + 2 half-life + recency + entropy = 17
            temporal_dim_per_domain = 17
            base = -900000
            for i, domain in enumerate(self.seq_domains):
                self.user_dense_schema.add(base - i, temporal_dim_per_domain)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        if self.is_training:
            ...
            click_labels = (label_type >= 1).astype(np.int64)
        else:
        ...
        if self.enable_temporal_features and self.seq_domains:
            agg_features = []
            for domain in self.seq_domains:
                tb = result[f'{domain}_time_bucket'].numpy()
                ln = result[f'{domain}_len'].numpy()
                agg_features.append(build_temporal_features(tb, ln))
            temporal_extra = np.concatenate(agg_features, axis=1).astype(np.float32)
            result['user_dense_feats'] = torch.cat(
                [result['user_dense_feats'], torch.from_numpy(temporal_extra)], dim=1
            )
        ...
        if current_dense_dim != expected_dense_dim:
            logging.warning(
                "user_dense_feats dim mismatch in dataset convert: current=%d expected=%d. "
                "Applying auto-align (truncate/pad) to prevent shape errors.",
                current_dense_dim, expected_dense_dim,
            )
            if current_dense_dim > expected_dense_dim:
                result['user_dense_feats'] = result['user_dense_feats'][:, :expected_dense_dim]
            else:
                pad = torch.zeros(
                    B, expected_dense_dim - current_dense_dim, dtype=result['user_dense_feats'].dtype
                )
                result['user_dense_feats'] = torch.cat([result['user_dense_feats'], pad], dim=1)
